...
async def private_receive_handler(c: Client, m: Message):
    ...
    if check_pass== None:
        await m.reply_text("Login first using /login cmd \n don\'t know the pass? request it from @mrstrange_genuine")
        return
    ...
    if Var.UPDATES_CHANNEL != "None":
        try:
            ...
            if user.status == "kicked":
                await c.send_message(
                    chat_id=m.chat.id,
                    text="__Sᴏʀʀʏ Sɪʀ, Yᴏᴜ ᴀʀᴇ Bᴀɴɴᴇᴅ ᴛᴏ ᴜsᴇ ᴍᴇ.__\n\n  **Cᴏɴᴛᴀᴄᴛ Dᴇᴠᴇʟᴏᴘᴇʀ @mrstrange_genuine ʜᴇ Wɪʟʟ Hᴇʟᴘ Yᴏᴜ**",
                    parse_mode="markdown",
            ...
            await c.send_message(
                chat_id=m.chat.id,
                text="**Sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ Wʀᴏɴɢ. Cᴏɴᴛᴀᴄᴛ ᴍʏ ʙᴏss** @mrstrange_genuine",
                parse_mode="markdown",
                disable_web_page_preview=True)
            return
    try:
